...
class LinearRegression(object):
    ...
    def feature_transform(self, X):
        ...
        X_transformed = np.column_stack([X, x0]) # or np.c_[dummy_X, v] https://stackoverflow.com/questions/20978757/how-to-append-a-vector-to-a-matrix-in-python
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
    ...
    def train_analytic(self, X, y):
        ...
        #############################################################################
        self.params['W'] = np.linalg.pinv(np.dot(X.T,X)).dot(X.T).dot(y)
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
    ...
    def loss(self, X, y=None):
        ...
        #############################################################################
        dJ = np.multiply(loss,X)
        # dJ = np.sum(dJ,axis=1)
        grads['W'] = np.sum(dJ,axis=1)#.reshape(N,1) # np.append(grads['W'],[np.sum(dJ,axis=1)],axis=0)
